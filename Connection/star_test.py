...
class Peripherial:
    ...
    def failing(self):
        while(True):
            ...
            self.status.append("COMPUTER  {self.index} failing after {time_to_fail} interrupted {self.current_job}")
            self.failed = True
            self.current_job = 0
            time.sleep(self.reboot)
            self.failed = False
            self.status.append("COMPUTER  {self.index}  restarted")

    def assign_job(self, job):
        self.status.append("COMPUTER  {self.index}  job assigned {job}")
        ...
        while(True):
            if self.failed == False and self.occupied == False and self.current_job != 0:
                ...
                if self.current_job == 1:
                    self.sum_vec(A, B)
                    self.status.append("COMPUTER  {self.index}  sum vectors 1")
                    self.current_job = 0
                elif self.current_job == 2:
                    self.dot_product(A, B)
                    self.status.append("COMPUTER  {self.index}  dot product 2")
                    self.current_job = 0
                    ...
                    self.multiply(X,Y)
                    self.status.append("COMPUTER  {self.index}  multiply matrix 3")
                    self.current_job = 0
                else:
...
class Central:
    def __init__(self, num_p, alpha, delta, reboot, gamma):
        print(num_p, alpha, delta, reboot, gamma)
    ...
    def fails(self):
        while(self.turned_on):
            ...
            time.sleep(time_to_fail)
            self.status.append("CENTRAL FAILING...failing central")
            self.failed = True
            self.status.append("CENTRAL FAILING...restarted ")
            time.sleep(self.reboot)
            self.failed = False
    ...
    def intialize_job_generator(self):
        ...
        while(self.turned_on):
            ...
            self.status.append("CENTRAL INITIALIZING...append job {j} waited this amount of time {s}")
            self.do.append(j)
            ...
            if self.failed == False:
                ...
                if self.do:
                    ...
                    if self.peripherial[self.counter].occupied == False:
                        self.status.append("CENTRAL DISTRIBUTING... this is the {to_do} job, executed by {self.counter} computer ")
                        self.peripherial[self.counter].assign_job(to_do)
                        self.counter += 1
                    if self.counter == self.num_p:
                        self.counter = 0
                else:
                    self.status.append("CENTRAL DISTRIBUTING... NO JOB TO BE DONE")
